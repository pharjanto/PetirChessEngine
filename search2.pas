...
{    if depth>=maxdepth then
    ...
    if (rlevel>=3) and (beta<_INFINITY-100) and not nullmove
    ...
    begin
      ...
      begin
        ...
        if tempep=0 then
        begin
          tempep:=ipopcount(data.whitepieces);
        end;
      end
      else
      begin
        ...
        if tempep=0 then
        begin
          tempep:=ipopcount(data.blackpieces);
        end;
      end;
      if tempep>=3 then
      begin
        ...
        if (data.nilai_perwira_putih<=8) and (data.nilai_perwira_hitam<=8) and (rlevel>8) then
          inc(newlevel,8);
...
{      inc(anu7);
      ...
      begin
        ...
        1:
          begin
              if giliran=_SISIPUTIH then
                tempi:=data.materialscore+captured_value(sto,temp)
              else
                tempi:=(-data.materialscore)+captured_value(sto,temp);
